...
void tcp_out(tcp_conn_t *tcp_conn, buf_t *buf, uint16_t src_port, uint8_t *dst_ip, uint16_t dst_port, uint8_t flags) {
    ...
    tcp_hdr->doff = (uint32_t) ((sizeof(tcp_hdr_t) / 4) << 4);
    tcp_hdr->flags = flags;
...
void tcp_in(buf_t *buf, uint8_t *src_ip) {
    ...
    switch (tcp_conn->state) {
        ...
        case TCP_STATE_ESTABLISHED:
            ...
            uint16_t payload_len = buf->len - tcp_hdr_sz;
    ...
    if (buf->len - tcp_hdr_sz > 0) {
        tcp_handler_t *tcp_handler = (tcp_handler_t *)map_get(&tcp_handler_table, &host_port);
        if (tcp_handler == NULL) {
            buf_add_header(buf, sizeof(ip_hdr_t));
            icmp_unreachable(buf, remote_ip, ICMP_CODE_PORT_UNREACH);
        } else {
            buf_remove_header(buf, tcp_hdr_sz);
            (*tcp_handler)(tcp_conn, buf->data, buf->len, remote_ip, remote_port);
        }
    }
